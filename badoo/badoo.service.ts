...
export class BadooService {
  ...
  login = async (options: { login?: string, password?: string, rememberMe?: boolean, isDebug?: boolean | string }): Promise<Page> => {
    ...
    await tryNavigate(page, 'https://badoo.com/signin');
    ...
    await page.waitForSelector('[data-choice="yes"]', {
      visible: true,
      timeout: 300000
    });
  ...
  async autoLikes(options: {
    ...
    // Откроем новую страницу
    await tryNavigate(page, 'https://badoo.com/encounters');

    let counter = 0;

    while (true) {
      try{
        if (await isExist(page, '.ovl__frame.js-ovl-wrap', 50)) {
          await page.click('.btn.js-continue');
        ...
        page.content().then(content => {
          ...
          const profileName = $('.profile-header__user.header-1').text()?.trim().replace(/ /g, '');

          console.log(`${counter++}: ${profileName}`)
        });
  ...
  async getAllCategoriesPurchases(options: {
    ...
    // Откроем новую страницу
    let url = 'https://api.ipify.org'; //  'https://www.grainger.com';
