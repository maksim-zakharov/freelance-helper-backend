...
import { resolveCaptcha } from "../captcha";
...
export class UpworkService {
  ...
  async getSummaryByCipherText(ciphertext: string) {

    if (!this._browser) {
      this._browser = await createBrowser(true);
    }

    const page = await createPage(this._browser, false);

    // Откроем новую страницу
    await tryNavigate(page, `https://www.upwork.com/jobs/${ciphertext}`);
    // const content: string = await axios.get(`https://www.upwork.com/jobs/${ciphertext}`, {
    //   headers: {
    ...
    const notCaptcha = await isExist(page, '#main');
    if (!notCaptcha) {
    ...
    const [workload, duration, tierText, hourlyBudgetText] = $('.cfe-ui-job-features .header').toArray().map(li => $(li).text().trim());

    let clientActivity = $('.list-unstyled:nth-child(2) li').toArray().map(li => $(li).text().trim());

    clientActivity = clientActivity.map(ca => ca.split(':')).reduce((acc, [label, value]) => ({
      ...acc,
      [label]: value?.split('\n')?.slice(-1)?.[0]?.trim()
    }), {});
    ...
    return {
      job: {
        ...
        hourlyBudgetText,
        clientActivity: {
          invitationsSent: +clientActivity["Invites sent"] || 0,
          totalApplicants: +clientActivity["Proposals"]?.split(' ')?.[0] || 0,
          unansweredInvites: +clientActivity["Unanswered invites"] || 0,
          totalInvitedToInterview: +clientActivity["Interviewing"] || 0,
          totalHired: +clientActivity["Hired"] || 0,
        }
      }
    };
  }
